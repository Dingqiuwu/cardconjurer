...
function autoFrame() {
	...
	if (types.includes('land') || types.includes('地')) {
		...
		// Look for Chinese "add mana" wording.
		lines.forEach(function(line) {
			var addIndex = line.indexOf('加');
			if (addIndex != -1) {
				var upToAdd = line.substring(addIndex + 1);
				['W', 'U', 'B', 'R', 'G'].forEach(function (color) {
					if (upToAdd.includes('{' + color.toLowerCase() + '}')) {
						colors.push(color);
					}
		...
		if (!colors.includes('W') && (rules.includes('plains') || types.includes('plains') || rules.includes('平原') || types.includes('平原'))) {
			colors.push('W');
		}
		if (!colors.includes('U') && (rules.includes('island') || types.includes('island') || rules.includes('海岛') || types.includes('海岛'))) {
			colors.push('U');
		}
		if (!colors.includes('B') && (rules.includes('swamp') || types.includes('swamp') || rules.includes('沼泽') || types.includes('沼泽'))) {
			colors.push('B');
		}
		if (!colors.includes('R') && (rules.includes('mountain') || types.includes('mountain') || rules.includes('山脉') || types.includes('山脉'))) {
			colors.push('R');
		}
		if (!colors.includes('G') && (rules.includes('forest') || types.includes('forest') || rules.includes('树林') || types.includes('树林'))) {
			colors.push('G');
		}

		// Handle fetch lands and special cases
		if ((rules.includes('search') || rules.includes('搜寻')) && colors.length == 0) {
			// Tutor effects that put cards in hand don't add colors
			if ((rules.includes('into your hand') || rules.includes('置于你手上')) ||
				((rules.includes('tapped') && !rules.includes('enters the battlefield tapped') && !rules.includes('untap')) ||
				(rules.includes('横置') && !rules.includes('横置进入战场') && !rules.includes('重置')))) {
				colors = [];
		...
		if (colors.length === 0 && (rules.includes('any color') || rules.includes('any one color') || rules.includes('choose a color') || rules.includes('any combination of colors') || rules.includes('任意颜色') || rules.includes('任意颜色的单色') || rules.includes('选择一种颜色') || rules.includes('其颜色组合由你选择'))) {
			colors = ['W', 'U', 'B', 'R', 'G'];
		}
		...
		if (rules.includes('devoid') || rules.includes('虚色')) {
			colors = [];
		}
	}
